...
const styles = require('./content.css') // eslint-disable-line no-unused-vars
...
function canElementBeTypedIn(el) {
	...
	return el.isContentEditable ||
		(!el.readOnly && (
			tagName === 'textarea' ||
			(tagName === 'input' && typeCanBeTypedIn)
		))
}
	...
	for (const hint of state.hints) {
		...
		const top = Math.max(
			...
			Math.min(
				...
				winHeight - cache.hintHeight
			)
		)
		const left = Math.max(
			0,
			Math.round(
				targetPos.left - cache.hintWidth - hintCharWidth - 2
			)
		)
